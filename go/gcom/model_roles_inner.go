/*
GCOM API

Grafana.com API (public).  Looking for GCOM API client packages? You can find them at [grafana-com-public-clients](https://github.com/grafana/grafana-com-public-clients) repository.  If you have any questions, please contact support in the Grafana Cloud UI.  This spec is in *Beta* stage, so use it with caution: - Not all endpoint responses are properly typed for the time being. - Some request parameter types may not be precise

API version: public
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package gcom

import (
	"encoding/json"
)

// checks if the RolesInner type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &RolesInner{}

// RolesInner struct for RolesInner
type RolesInner struct {
	Role *Role `json:"role,omitempty"`
	// Default assignments of the role to Grafana basic roles (`Viewer`, `Editor`, `Admin`, `Grafana Admin`).
	Grants               []string `json:"grants,omitempty"`
	AdditionalProperties map[string]interface{}
}

type _RolesInner RolesInner

// NewRolesInner instantiates a new RolesInner object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewRolesInner() *RolesInner {
	this := RolesInner{}
	return &this
}

// NewRolesInnerWithDefaults instantiates a new RolesInner object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewRolesInnerWithDefaults() *RolesInner {
	this := RolesInner{}
	return &this
}

// GetRole returns the Role field value if set, zero value otherwise.
func (o *RolesInner) GetRole() Role {
	if o == nil || IsNil(o.Role) {
		var ret Role
		return ret
	}
	return *o.Role
}

// GetRoleOk returns a tuple with the Role field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *RolesInner) GetRoleOk() (*Role, bool) {
	if o == nil || IsNil(o.Role) {
		return nil, false
	}
	return o.Role, true
}

// HasRole returns a boolean if a field has been set.
func (o *RolesInner) HasRole() bool {
	if o != nil && !IsNil(o.Role) {
		return true
	}

	return false
}

// SetRole gets a reference to the given Role and assigns it to the Role field.
func (o *RolesInner) SetRole(v Role) {
	o.Role = &v
}

// GetGrants returns the Grants field value if set, zero value otherwise.
func (o *RolesInner) GetGrants() []string {
	if o == nil || IsNil(o.Grants) {
		var ret []string
		return ret
	}
	return o.Grants
}

// GetGrantsOk returns a tuple with the Grants field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *RolesInner) GetGrantsOk() ([]string, bool) {
	if o == nil || IsNil(o.Grants) {
		return nil, false
	}
	return o.Grants, true
}

// HasGrants returns a boolean if a field has been set.
func (o *RolesInner) HasGrants() bool {
	if o != nil && !IsNil(o.Grants) {
		return true
	}

	return false
}

// SetGrants gets a reference to the given []string and assigns it to the Grants field.
func (o *RolesInner) SetGrants(v []string) {
	o.Grants = v
}

func (o RolesInner) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o RolesInner) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Role) {
		toSerialize["role"] = o.Role
	}
	if !IsNil(o.Grants) {
		toSerialize["grants"] = o.Grants
	}

	for key, value := range o.AdditionalProperties {
		toSerialize[key] = value
	}

	return toSerialize, nil
}

func (o *RolesInner) UnmarshalJSON(data []byte) (err error) {
	varRolesInner := _RolesInner{}

	err = json.Unmarshal(data, &varRolesInner)

	if err != nil {
		return err
	}

	*o = RolesInner(varRolesInner)

	additionalProperties := make(map[string]interface{})

	if err = json.Unmarshal(data, &additionalProperties); err == nil {
		delete(additionalProperties, "role")
		delete(additionalProperties, "grants")
		o.AdditionalProperties = additionalProperties
	}

	return err
}

type NullableRolesInner struct {
	value *RolesInner
	isSet bool
}

func (v NullableRolesInner) Get() *RolesInner {
	return v.value
}

func (v *NullableRolesInner) Set(val *RolesInner) {
	v.value = val
	v.isSet = true
}

func (v NullableRolesInner) IsSet() bool {
	return v.isSet
}

func (v *NullableRolesInner) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableRolesInner(val *RolesInner) *NullableRolesInner {
	return &NullableRolesInner{value: val, isSet: true}
}

func (v NullableRolesInner) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableRolesInner) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
